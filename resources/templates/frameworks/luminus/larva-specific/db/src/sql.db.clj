(ns <<project-ns>>.db.core<% if embedded-db %>
  (:require
    ...
    [larva.db.utils :refer [make-queries-from-dirs]])<% endif %><% ifequal db-type "postgres" %>
  (:require
...
(make-queries-from-dirs "sql" {:connection conn})<% endifequal %><% ifequal db-type "h2"%>
(def conn
...
(make-queries-from-dirs "sql" {:connection conn})<% endifequal %><% else %>
...
(conman/bind-connection *db* <<for queries-files>>)
